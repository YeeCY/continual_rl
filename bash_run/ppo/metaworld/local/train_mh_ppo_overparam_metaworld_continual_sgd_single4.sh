...
for env_names in "${all_env_names[@]}"; do
  for seed in "${seeds[@]}"; do
    ...
    python $PROJECT_DIR/src/train_ppo.py \
      ...
      --seed $seed \
      --save_video \
      --work_dir $PROJECT_DIR/vec_logs/ppo_mlp_overparam_metaworld_single/sgd/$env_names/$seed \
      > $PROJECT_DIR/terminal_logs/ppo_mlp_overparam_metaworld_single-sgd-"$env_names"-seed"$seed".log 2>&1 &
  done
done
