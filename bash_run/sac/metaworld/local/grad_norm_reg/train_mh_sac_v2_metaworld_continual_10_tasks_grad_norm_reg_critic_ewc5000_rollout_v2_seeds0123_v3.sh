...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/src/train_sac.py \
    ...
    --sac_ewc_estimate_fisher_sample_src rollout \
    --sac_ewc_critic_grad_norm_reg_coeff 1.0 \
    --seed $seed \
    --work_dir $PROJECT_DIR/vec_logs/grad_norm_reg_critic_mh_sac_mlp_v2_metaworld_10_tasks/ewc_lambda5000_rollout/$seed \
    > $PROJECT_DIR/terminal_logs/grad_norm_reg_critic_mh_sac_mlp_v2_metaworld_10_tasks-ewc_lambda5000_rollout-seed"$seed".log 2>&1 &
done
