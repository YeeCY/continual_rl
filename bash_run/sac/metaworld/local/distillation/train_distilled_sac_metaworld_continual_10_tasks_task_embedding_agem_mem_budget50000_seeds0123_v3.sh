...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/src/train_sac.py \
    --env_names \
      ...
      window-open-v2 \
    --env_type metaworld \
    --algo agem_task_embedding_distilled_actor_sac_mlp \
    ...
    --seed $seed \
    --work_dir $PROJECT_DIR/vec_logs/distilled_sac_mlp_metaworld_10_tasks/distillation_agem_memory_budget50000/$seed \
    > $PROJECT_DIR/terminal_logs/distilled_sac_mlp_metaworld_10_tasks-distillation_agem_memory_budget50000-seed"$seed".log 2>&1 &
done
