...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/src/train_sac.py \
    --env_names \
      ...
      window-open-v2 \
    --env_type metaworld \
    --algo distilled_actor_mi_sac_mlp \
    ...
    --seed $seed \
    --work_dir $PROJECT_DIR/vec_logs/distilled_mi_sac_mlp_metaworld_10_tasks/distilled_actor_no_forgetting_reg/$seed \
    > $PROJECT_DIR/terminal_logs/distilled_mi_sac_mlp_metaworld_10_tasks-distilled_actor_no_forgetting_reg-seed"$seed".log 2>&1 &
done
