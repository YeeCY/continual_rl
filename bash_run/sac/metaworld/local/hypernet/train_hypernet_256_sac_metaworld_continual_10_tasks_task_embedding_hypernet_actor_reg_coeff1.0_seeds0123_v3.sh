...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/src/train_sac.py \
    ...
    --sac_hypernet_reg_coeff 1.0 \
    --sac_hypernet_on_the_fly_reg False \
    --sac_hypernet_first_order True \
    --seed $seed \
    --work_dir $PROJECT_DIR/vec_logs/hypernet_sac_mlp_metaworld_10_tasks/task_embedding_hypernet_actor_256_reg_coeff1.0/$seed \
    > $PROJECT_DIR/terminal_logs/hypernet_sac_mlp_metaworld_10_tasks-task_embedding_hypernet_actor_256_reg_coeff1.0-seed"$seed".log 2>&1 &
done
