...
_ACTION_SPEC_MUST_BE_BOUNDED_ARRAY = (
    "`environment.action_spec()` must return a single `BoundedArray`, got: {}.")
...
class Wrapper(dm_env.Environment):
  ...
  def __init__(self, env, minimum, maximum):
    ...
    Raises:
      ValueError: If `environment.action_spec()` is not a single `BoundedArray`.
      ValueError: If `environment.action_spec()` has non-finite bounds.
    ...
    validate(orig_minimum, "environment.action_spec().minimum")
    validate(orig_maximum, "environment.action_spec().maximum")
