...
class ColorWrapper(gym.Wrapper):
    ...
    def __init__(self, env, mode):
        assert isinstance(env, FrameStack), 'wrapped environment must be a framestack'
    ...
    def _load_colors(self):
        ...
        self._colors = torch.load('src/environment/data/{}.pt'.format(self._mode.replace('eval_', '')))

    def get_random_color(self):
        assert len(self._colors) >= 100, 'environment must include at least 100 colors'
    ...
    def _get_dmc_wrapper(self):
        _env = self.env
        while not isinstance(_env, dmc2gym.wrappers.DMCWrapper) and hasattr(_env, 'environment'):
            _env = _env.env
    ...
    def _reload_physics(self, xml_string, assets=None):
        _env = self.env
        while not hasattr(_env, '_physics') and hasattr(_env, 'environment'):
            _env = _env.env
    ...
    def __init__(self, env, mode):
        ...
        if 'video' in mode:
            ...
            if not self._video.endswith('.mp4'):
                self._video += '.mp4'
            self._video = os.path.join('src/environment/data', self._video)
