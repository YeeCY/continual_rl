...
def main(args):
    ...
    for task_name in args.env_names:
        ...
        for param in agent.actor.main_parameters():
            ori_actor_params.append(param.detach().clone().flatten())
        ori_actor_params = torch.cat(ori_actor_params).to(args.device)

        # pad zero to then end of actor params
        num_chunks = np.ceil(ori_actor_params.shape[0] / chunk_size).astype(int)
        actor_params = torch.cat([
            ori_actor_params,
            torch.zeros(num_chunks * chunk_size - ori_actor_params.shape[0],
                        device=args.device)
        ])
        actor_params = actor_params.reshape(-1, chunk_size)
        ...
        with torch.no_grad():
            ...
            sample = model.sample_latent_variable()  # a full sample returns latent x across all N
            preds = likelihood(model(sample))
            means = preds.mean.T
        # means = torch.cat(means)
        # nn_means = torch.cat(nn_means)
        print('Test MAE: {}'.format(
            utils.to_np(torch.mean(torch.abs(means - actor_params))),
        ))

        # print('Test MAE: {}, NN MAE: {}'.format(
        #     utils.to_np(torch.mean(torch.abs(means - actor_params))),
        #     utils.to_np(torch.mean(torch.abs(nn_means - actor_params)))
        # ))
        ...
        with torch.no_grad():
            ...
            pred_params = preds.sample().T.reshape(-1)
