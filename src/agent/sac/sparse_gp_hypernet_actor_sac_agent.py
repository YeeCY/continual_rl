...
from agent.nets.sparse_gp import SacActorMainNetMlp, SacSparseGPActorHyperNetMlp
from agent.network import SacCriticMlp


class SparseGPHyperNetActorSacMlpAgent(SacMlpAgent):
    def __init__(
            self,
            obs_shape,
            action_shape,
        ...
        # self.gp_warmup_epochs = gp_warmup_epochs
    ...
    def _setup_agent(self):
        ...
        self.hypernet = SacSparseGPActorHyperNetMlp(
            num_tasks, actor_shapes, self.gp_num_inducing_points
        ).to(self.device)
