...
class SacActorMainNetMlp(nn.Module):
    ...
    def __init__(self, obs_shape, action_shape, hidden_dim, log_std_min, log_std_max,
        ...
        # self.apply(weight_init)  # (cyzheng): no weight for main network now

    # @property
    # def weights(self):
    #     # FIXME (cyzheng): weights are generated by hyper net now
    ...
    def forward(self, obs, compute_pi=True, compute_log_pi=True, weights=None):
        ...
        #                        bias=weights['output_layer.bias']).chunk(2, dim=-1)
...
class SacTaskEmbeddingActorHyperNetMlp(nn.Module):
    def __init__(self, num_tasks, actor_shapes, hidden_dim,
                 task_embedding_dim, act_func=torch.relu):
        super().__init__()
